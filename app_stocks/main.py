...
from .database.models import Stock, StockRequest
...
#
@app.post("/stock")
async def stock_add(stock_request: StockRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    """
...
#
@app.post("/delete")
async def stock_delete(stock_request: StockRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    """
    delete specified ticker from the database
    """

    db.query(Stock).filter(Stock.symbol ==stock_request.symbol).delete()
    db.commit()

    return {
        "code": "success",
        "message": "stock was deleted to the database"
    }